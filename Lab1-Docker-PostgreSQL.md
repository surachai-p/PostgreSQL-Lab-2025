Named Volume	Docker จะสร้างพื้นที่เก็บข้อมูลภายใน Docker เอง (จัดการโดย Docker)	ใช้เก็บฐานข้อมูล เช่น /var/lib/postgresql/data ปลอดภัยและง่ายต่อการ backup/restore เพราะไม่ขึ้นอยู่กับ path บน host
Bind Mount	เชื่อม folder บน host machine ไปยัง container	เหมาะกับการพัฒนา หรือใช้ config/postgres.conf ที่ต้องแก้บ่อย ๆ แต่ต้องระวัง permission และความเข้ากันของ OS
...
2. ทำไม shared_buffers ควรตั้ง ~25% ของ RAM
shared_buffers คือ memory ของ PostgreSQL สำหรับเก็บข้อมูลที่อ่านจาก disk
การตั้งให้มากเกินไป → container อาจใช้ RAM เกินความจำเป็น → OS/ระบบอื่นล่ม
การตั้งน้อยเกินไป → PostgreSQL ต้องอ่าน disk บ่อย → ประสิทธิภาพลด
กฎง่าย ๆ:
shared_buffers ≈ 25% ของ RAM
เช่น RAM 4GB → shared_buffers ≈ 1GB
เหตุผล: PostgreSQL จะ cache ข้อมูลใน memory ทำให้ query เร็วขึ้น และลด I/O จาก disk

3. การใช้ Schema ช่วยในการจัดการฐานข้อมูลขนาดใหญ่
Schema = การแบ่ง logical namespace ภายใน database
ประโยชน์:
แยกตารางตามฟังก์ชัน: เช่น sales.customers, hr.employees
จัดการสิทธิ์ได้ง่าย: ให้ user บางคนเข้าถึง schema เฉพาะ
ลดความซ้ำซ้อนของชื่อตาราง: สามารถมีตารางชื่อเดียวกันใน schema ต่างกันได้
ช่วยในการ maintain database ขนาดใหญ่: จัดกลุ่มข้อมูลเป็น logical unit

ตัวอย่าง:

CREATE SCHEMA sales;
CREATE TABLE sales.customers (id SERIAL PRIMARY KEY, name VARCHAR(50));
CREATE SCHEMA hr;
CREATE TABLE hr.employees (id SERIAL PRIMARY KEY, name VARCHAR(50));

4. ประโยชน์ของการใช้ Docker สำหรับ Database Development
ไม่กระทบระบบหลัก: สามารถสร้าง PostgreSQL container ใหม่ได้หลาย instance
ง่ายต่อการ setup & cleanup:
docker run postgres → ready ใช้
docker rm -v container → ลบข้อมูลและ container จบ
จำลอง environment production:
ตั้ง version, config, resource limits เหมือน production
แชร์และ collaborate:
developer คนอื่นสามารถใช้ docker-compose เพื่อสร้าง environment เดียวกัน
ทดลอง config parameter: เช่น shared_buffers, work_mem โดยไม่กระทบเครื่องหลัก
